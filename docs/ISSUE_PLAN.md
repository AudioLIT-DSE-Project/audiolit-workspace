...
| LIT-208 | CREMA-D/RAVDESS demo subset | FR2 | Tharusha | 🟡 | LIT-123 ✅ | `CremaDLoader` + `RavdessLoader` in `dataset_ingestion.py`, wired into `CORPUS_REGISTRY`. Labels normalised onto a canonical `EMOTION_LABELS` vocabulary that **matches the SER checkpoint's spellings** (`fearful`/`surprised`, not `fear`/`surprise`) — a mismatch there would silently score every fear clip wrong. RAVDESS song channel excluded by default; CREMA-D demographics parsed for the FR15 work. `demo_clips_by_emotion()` gives the balanced per-class sample for the mid-eval walkthrough. **ESD deliberately left unwired** — out of this issue's scope; `get_loader("esd")` still raises naming LIT-208. **Unblocks LIT-224's outstanding accuracy check, and LIT-206.** |
