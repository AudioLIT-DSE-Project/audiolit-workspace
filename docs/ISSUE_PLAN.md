...
| LIT-237 | Close FR2 dataset-management compliance gaps (integrity, footprint, licence) | FR2 | Ravindu | 🟡 In Review | LIT-123, LIT-235 | Direct audit of FR2 (`docs/FR2_AUDIT.md`) found the seven-corpus ingestion core solid but three sub-clauses unwired: FR2.1 (`is_silent`/integrity had zero production callers — now wired into `DatasetLoader.validated_stream`/`check_integrity`, used by the listing path (shallow) and `accent_bias_profiler.load_accent_cohorts` (deep)); FR2.2 (`/{dataset}/metadata` materialized whole catalogs — now capped via `settings.DATASET_METADATA_ROW_CAP` + new `measure_footprint()`/`GET /datasets/footprint`/startup warning); FR2.3 (licence retained in the domain model but dropped by the API and never rendered — now in `GET /datasets/list`'s `licenses` key + new `DatasetLicenseNotice.tsx` banner in `AudioDatasetPanel.tsx`). Also fixed: LibriSpeech offered in the dropdown with no data provisioned (new `is_corpus_available` probe → `available` key, `Toolbar.tsx` greys it out), and a dead unreachable RAVDESS duration branch in `dataset_service.py`. **Deliberately deferred, not done here:** collapsing the Common Voice legacy CSV path onto the generic registry row shape (regression risk — its CSV has richer columns), and a cached filename→path index for `_registry_resolve_file` (perf-only, no compliance impact). |
...
| ID | Title | FR | Assignee | Status | Blocked by | Notes |
|---|---|---|---|---|---|---|
| LIT-185 | Projection-space lasso handler | FR11 | Tharusha | 🟡 In flight | **LIT-207** | **PR #91 opened.** Implemented lasso & box selection event handler in `Frontend/src/components/visualization/EmbeddingPlot.tsx`. |
...
| ID | Title | FR | Assignee | Status | Blocked by | Notes |
|---|---|---|---|---|---|---|
| LIT-169 | Quantitative faithfulness checking | FR16 | Tharusha | 🟡 In flight | **LIT-126** (needs ≥1 attribution method) | **PR #88 opened.** Implemented in `Backend/app/domain/perturbation_service.py` & verified with `test_faithfulness.py`. |
