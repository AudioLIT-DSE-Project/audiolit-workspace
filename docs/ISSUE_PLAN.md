...
| LIT-211 | Forward/attention hook registration | FR1 | Tharusha | 🟡 In Progress | LIT-207 (loose — same PR is fine) | Unlocks all attribution work (FR8/FR9/FR17). **Note:** Linear's own blockedBy relations list LIT-124 (stale — superseded duplicate) and LIT-210 (not actually required — `HookManager` was built against already-instantiated Whisper/Wav2Vec2 models in the existing `model_loader_service.py`, decoupled from LIT-210's registry/resolver work). Flagged for correction, not yet fixed in Linear. |
