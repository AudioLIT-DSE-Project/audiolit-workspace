...
* ~~TBD-1: final selection of the fine-tuned SER classifier checkpoint, pending comparison against the inherited ECHO classifier.~~ **RESOLVED (LIT-224, 2026-08-06).** The inherited ECHO checkpoint `r-f/wav2vec-english-speech-emotion-recognition` was rejected: it publishes no safetensors (so the Model Registry refuses it under constraint C3, and every SER call raised an error), and its weights carry a custom `classifier.dense`/`classifier.out_proj` head that `Wav2Vec2ForSequenceClassification` cannot load — all four head tensors were randomly initialised, making predictions chance-level noise that varied with the random seed. Selected default: **`firdhokk/speech-emotion-recognition-with-facebook-wav2vec2-large-xlsr-53`**, pinned at revision `611e6db8ee667aa07fe66596f9fc761e036ff5b9`. It ships safetensors, loads its head with zero missing keys, is deterministic across seeds, and covers all six FR6.1 categories plus surprise (labelled `fearful`/`surprised`). Alternatives evaluated and rejected: `superb/wav2vec2-base-superb-er` (head loads but only 4 classes — fails FR6.1); `ehcalabres/wav2vec2-lg-xlsr-en-speech-emotion-recognition` and `Dpngtm/wav2vec2-emotion-recognition` (declare `Wav2Vec2ForSequenceClassification` but ship the same custom head, so they random-initialise identically); `harshit345`, `m3hrdadfi`, `Aniemore`, `audeering` (custom `Wav2Vec2ForSpeechClassification` class, non-English, or dimensional rather than categorical). **Still outstanding:** accuracy on known-label clips could not be measured — that needs the CREMA-D/RAVDESS subset from LIT-208, which is not yet ingested.
