...
| LIT-183 | High-saliency masking engine | FR16 | Tharusha | 🟡 In flight | LIT-169 (parent) | **PR #89 opened.** Implemented `HighSaliencyMaskingEngine` in `Backend/app/domain/perturbation_service.py` & verified with `test_high_saliency_masking.py`. |
| LIT-184 | Downstream degradation scoring | FR16 | Tharusha | 🟡 In flight | LIT-169 (parent) | **PR #90 opened.** Implemented `evaluate_downstream_degradation` in `Backend/app/domain/perturbation_service.py` & verified with `test_degradation_scoring.py`. |
